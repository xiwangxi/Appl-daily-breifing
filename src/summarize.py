...
id/title/summary/published，summary 是原始报道的简介，可能为空)，以及 context（大盘指数/隔夜
全球市场（重点是日本、韩国、台湾这几个半导体权重股集中的市场）/半导体板块个股行情/近期3天内的
宏观日历事件(FOMC会议、CPI、PPI、非农就业)/自选清单近期财报日期)，请完成：
1. 去重：同一事件的多篇报道只保留信息量最大/来源最权威的一条。
...
4. 最多保留 8 条。
5. 基于这些新闻 + context 里的指数/隔夜市场/半导体板块/宏观日历/财报数据，生成"今日市场关注点"
   的中英文各一句（today_focus_zh 不超过50字，today_focus_en 一句话）。重点关注：
   a) 亚洲(日韩台)股市和半导体板块今天有没有大的异动（涨跌超过2%算比较明显），如果有要点出来；
   b) 今天或近几天有没有 FOMC/CPI/PPI/非农 等重要数据要公布，或者自选清单里有公司要发财报；
   c) 结合新闻挑1-2件今天最值得注意的事。三者里挑最值得说的1-2条即可，不用面面俱到。
